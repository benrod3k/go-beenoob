...
func (ethash *Ethash) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, fulldag bool) error {
	...
	if fulldag {
		...
		if dataset.generated() {
			digest, result = hashimotoFull(header.Number.Uint64(), dataset.dataset, ethash.SealHash(header).Bytes(), header.Nonce.Uint64())
	...
	if !fulldag {
		...
		digest, result = hashimotoLight(header.Number.Uint64(), size, cache.cache, ethash.SealHash(header).Bytes(), header.Nonce.Uint64())
...
func (ethash *Ethash) SealHash(header *types.Header) (hash common.Hash) {
	...
	if header.Number.Uint64() < 300000 {
		// CPU mining
